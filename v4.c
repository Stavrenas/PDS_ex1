...
int find_common( uint32_t * const row, uint32_t * const col,uint32_t i, uint32_t j){
	...
	int t=0;
				 for(uint32_t i2=col[i-1]; i2<col[i] ; i2++){  
				     //go to i_th row and multiply with j_th row (summetric). The result of each multiplication will be 1 if both elements are nnz
					 //So, for every nz element in i_th row i check if there is a nnz element in jth row that belongs in the same column
	...
	void V4( uint32_t * const row, uint32_t * const col, uint32_t  n){
		...
		uint32_t j,t;
		uint32_t k=0; //index of each element.

       for(uint32_t i=1; i<=n; i++){
		   
			   //i will do the multiplication only if A[i][j]==1, so i just iterate through 
			   // the non zero elements of A -> this implements the Hadamard product 
			   
			   for(uint32_t i1=col[i-1]; i1<col[i]; i1++,k++){	 //go to each nz element in row i->implement i_th row * j_th col 
				   j=row[i1];

				 C[k]=find_common(row,col,i,j);
...
void V4long( uint32_t * const row, uint32_t * const col, uint32_t  n){
		uint32_t *triangles= (uint32_t*)calloc(n, sizeof(uint32_t)) ;
		uint32_t *C= (uint32_t*)calloc(col[n], sizeof(uint32_t)) ; //the value array equivalent in csc format
		uint32_t j,t,start;
		uint32_t k=0; //index of each element.
