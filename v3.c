...
)
{
	...
	int V3( uint32_t * const row, uint32_t * const col, uint32_t  n){
		long int *triangles= (long int*)calloc(n, sizeof(long int)) ;
		int j;

       for(int i=1; i<=n; i++){
		   
			   
			   for(int i1=col[i-1]; i1<col[i]; i1++){	 //go to each non zero element in ith row, lets asume A[i][j]
                   if(row[i1]==i)
					   continue;
				   j=row[i1];

				   int start=col[j-1];
				 for(int i2=col[i-1]; i2<col[i] ; i2++){     //go to each non zero element in jth row, lets asume A[j][k]

                    for(int i3=start; i3<col[j] && row[i3]<=row[i2];i3++){  //go to each non zero element in kth row and check if the element A[i][k] exists
